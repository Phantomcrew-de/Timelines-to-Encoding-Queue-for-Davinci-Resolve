...
class RenderGUI:
    def __init__(self, master):
        self.master = master
        master.title("Render GUI mit Projektwahl")
        ...
        tk.Label(master, text="Höhe:").grid(row=2, column=2, sticky='e')
        ...
        tk.Label(master, text="Bitrate (Mbit/s):").grid(row=3, column=2, sticky='e')
        ...
        tk.Checkbutton(master, text="Timeline In/Out Marker verwenden", variable=self.use_in_out).grid(row=5, column=0, columnspan=2, sticky='w')

        # Timeline-Suchmuster
        tk.Label(master, text="Timeline-Suchmuster:").grid(row=6, column=0, sticky='e')
        self.search_pattern = tk.Entry(master)
        self.search_pattern.grid(row=6, column=1)
        tk.Button(master, text="Filtern", command=self.select_timelines_by_pattern).grid(row=6, column=2)
        ...
        # Buttons und Log
        tk.Button(master, text="Zur Warteschlange hinzufügen", command=self.add_to_queue).grid(row=8, column=0, pady=10)
        tk.Button(master, text="Warteschlange löschen", command=self.clear_queue).grid(row=8, column=1, pady=10)
        tk.Button(master, text="Rendering starten", command=self.start_rendering).grid(row=8, column=2, pady=10)
    ...
    def on_project_change(self):
        ...
        if not project_manager.LoadProject(project_name):
            self.log(f"⚠ Projekt konnte nicht geladen werden: {project_name}\n")
            return
        self.current_project = project_manager.GetCurrentProject()
        self.update_formats()
        self.update_presets()
        self.load_timelines()
        self.log(f"📁 Projekt gewechselt zu: {project_name}\n")
    ...
    def add_to_queue(self):
        ...
        if not selected_timelines:
            messagebox.showwarning("Hinweis", "Bitte mindestens eine Timeline auswählen.")
            return
        ...
        if self.preset_var.get() == "Custom":
            try:
                ...
                messagebox.showerror("Fehler", "Bitte eine gültige Bitrate eingeben.")
                return
            settings.update({
    ...
    def enqueue_timelines(self, settings):
        ...
        for i in settings["selected_timelines"]:
            ...
            self.log(f"➕ {tl.GetName()} zur Warteschlange hinzugefügt\n")


    def clear_queue(self):
        if self.current_project:
            self.current_project.DeleteAllRenderJobs()
            self.log("🗑️ Warteschlange geleert\n")

    def start_rendering(self):
        if not self.current_project:
            self.log("❌ Kein Projekt geladen.")
            return
        self.log("🚀 Starte Rendering...\n")
        self.current_project.StartRendering()
        while self.current_project.IsRenderingInProgress():
            time.sleep(1)
        self.log("✅ Rendering abgeschlossen.\n")
    ...
    def load_last_preset(self):
        ...
        try:
            ...
            self.log(f"⚠ Fehler beim Laden des Presets: {e}\n")
