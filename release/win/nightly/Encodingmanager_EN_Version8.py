...
# Guarantee Fusion is present and UI API available
...
def main():
    ...
    for idx, name in timelines:
        cb = ui.CheckBox({"ID": f"tl_{idx}", "Text": name, "Checked": False, "Weight": 0})
        timeline_checkboxes.append(cb)
        status_lbl = ui.Label({"ID": f"status_{idx}", "Text": "Waiting", "Weight": 0})
        status_labels.append(status_lbl)
        timeline_vgroup.append(ui.HGroup([cb, status_lbl]))

    render_presets = project.GetRenderPresetList()

    window_layout = ui.VGroup([
        ui.Label({"Text": "Encoding Manager (Resolve Native UI)", "Alignment": {"AlignHCenter": True}, "Weight": 0}),
        ui.HGroup([
            ui.Label({"Text": "Render Preset:"}),
            ui.ComboBox({"ID": "preset_combo", "Editable": False, "Weight": 1, "CurrentIndex": 0,
                         "Items": render_presets}),
        ], {"Weight": 0}),
        ui.HGroup([
            ui.Label({"Text": "Output Directory:"}),
            ui.LineEdit({"ID": "output_dir", "PlaceholderText": "Select output directory...", "Weight": 2}),
            ui.Button({"ID": "browse_btn", "Text": "Browse", "Weight": 0}),
        ], {"Weight": 0}),
        ui.CheckBox({"ID": "use_io", "Text": "Use Timeline In/Out Markers", "Checked": False, "Weight": 0}),
        ui.Label({"Text": "Select Timelines to Render:"}),
        ui.VGroup(timeline_vgroup, {"ID": "timeline_vgroup", "Weight": 1, "Spacing": 2}),
        ui.ProgressBar({"ID": "progress", "Minimum": 0, "Maximum": max(len(timelines), 1), "Value": 0, "Weight": 0}),
        ui.TextEdit({"ID": "log", "ReadOnly": True, "Weight": 0, "MinimumHeight": 120}),
        ui.HGroup([
            ui.Button({"ID": "start_btn", "Text": "Start Rendering", "Weight": 0}),
            ui.Button({"ID": "quit_btn", "Text": "Quit", "Weight": 0}),
        ], {"Weight": 0}),
    ], {"Spacing": 8, "Weight": 1})
