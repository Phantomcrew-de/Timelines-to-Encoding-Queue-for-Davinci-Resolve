...
class RenderGUI:
    def __init__(self, master):
        self.master = master
        master.title("Render GUI with Project Selection")

        self.current_project = None

        # Projektwahl
        tk.Label(master, text="Project:").grid(row=0, column=0, sticky='e')
        ...
        # Render Preset
        tk.Label(master, text="Render Preset:").grid(row=0, column=2, sticky='e')
        self.preset_var = tk.StringVar()
        self.preset_menu = tk.OptionMenu(master, self.preset_var, "")
        self.preset_menu.grid(row=0, column=3, sticky='w')

        # Speicherort
        tk.Label(master, text="Output Directory:").grid(row=1, column=0, sticky='e')
        self.output_dir = tk.Entry(master, width=40)
        self.output_dir.grid(row=1, column=1)
        tk.Button(master, text="Browse", command=self.browse_dir).grid(row=1, column=2)
        ...
        tk.Label(master, text="Height:").grid(row=2, column=2, sticky='e')
        self.height = tk.Entry(master)
        self.height.grid(row=2, column=3)

        tk.Label(master, text="Framerate:").grid(row=3, column=0, sticky='e')
        ...
        # Format / Codec
        tk.Label(master, text="Format:").grid(row=4, column=0, sticky='e')
        ...
        tk.Checkbutton(master, text="Use Timeline In/Out Markers", variable=self.use_in_out).grid(row=5, column=0, columnspan=2, sticky='w')

        # Timeline-Suchmuster
        tk.Label(master, text="Timeline Search Pattern:").grid(row=6, column=0, sticky='e')
        self.search_pattern = tk.Entry(master)
        self.search_pattern.grid(row=6, column=1)
        tk.Button(master, text="Filter", command=self.select_timelines_by_pattern).grid(row=6, column=2)

        # Timeline-Auswahl
        self.timeline_vars = {}
        self.timeline_frame = tk.LabelFrame(master, text="Select Timelines")
        ...
        # Buttons und Log
        tk.Button(master, text="Add to Queue", command=self.add_to_queue).grid(row=8, column=0, pady=10)
        tk.Button(master, text="Clear Queue", command=self.clear_queue).grid(row=8, column=1, pady=10)
        tk.Button(master, text="Start Rendering", command=self.start_rendering).grid(row=8, column=2, pady=10)
    ...
    def on_project_change(self):
        ...
        if not project_manager.LoadProject(project_name):
            self.log(f"⚠ Could not load project: {project_name}\n")
            return
        self.current_project = project_manager.GetCurrentProject()
        self.update_formats()
        self.update_presets()
        self.load_timelines()
        self.log(f"📁 Switched project to: {project_name}\n")
    ...
    def add_to_queue(self):
        ...
        if not selected_timelines:
            messagebox.showwarning("Note", "Please select at least one timeline.")
            return
    ...
    def enqueue_timelines(self, settings):
        ...
        for i in settings["selected_timelines"]:
            ...
            self.log(f"➕ {tl.GetName()} added to queue\n")
    ...
    def start_rendering(self):
        if not self.current_project:
            self.log("❌ No project loaded.")
            return
        start_dt = datetime.now()
        self.log(f"🚀 Starting rendering at {start_dt.strftime('%H:%M:%S')}...\n")
        ...
        self.log(f"✅ Rendering finished at {end_dt.strftime('%H:%M:%S')}.\n")
        self.log(f"⏱ Duration: {minutes} Minuten {seconds} Sekunden\n")
    ...
    def save_last_preset(self, settings):
        try:
            ...
            self.log(f"⚠ Error beim Speichern: {e}\n")
    ...
    def load_last_preset(self):
        ...
        try:
            ...
            self.log(f"⚠ Error beim Laden des Presets: {e}\n")
