...
class RenderGUI:
    def __init__(self, master):
        self.master = master
        master.title("Renderinstellingen")

        self.current_project = project
        self.render_presets = project.GetRenderPresetList() if project else []

        tk.Label(master, text="Rendervoorinstelling:").grid(row=0, column=0, sticky='e')
        ...
        tk.Label(master, text="Tijdlijn zoekpatroon:").grid(row=1, column=0, sticky='e')
        self.search_pattern = tk.Entry(master)
        self.search_pattern.grid(row=1, column=1)
        tk.Button(master, text="Tijdlijnen filteren", command=self.select_timelines_by_pattern).grid(row=1, column=2)

        self.timeline_vars = {}
        self.status_labels = []
        self.timeline_frame = tk.LabelFrame(master, text="Tijdlijnen")
        ...
        tk.Label(master, text="Doelmap:").grid(row=3, column=0, sticky='e')
        self.output_dir = tk.Entry(master, width=40)
        self.output_dir.grid(row=3, column=1)
        tk.Button(master, text="Bladeren", command=self.browse_dir).grid(row=3, column=2)

        self.use_in_out = tk.IntVar()
        tk.Checkbutton(master, text="Gebruik In/Out-markeringen van tijdlijn", variable=self.use_in_out).grid(row=4, column=0, columnspan=2)
        ...
        tk.Button(master, text="Renderen starten", command=self.on_start).grid(row=6, column=1, pady=10)
    ...
    def load_timelines(self):
        ...
        for i in range(1, timeline_count + 1):
            ...
            if tl:
                ...
                cb.pack(side="left")
                status = tk.Label(row, text="Wachten", width=10)
    ...
    def on_start(self):
        ...
        if not selected_timelines:
            tkMessageBox.showwarning("Waarschuwing", "Selecteer ten minste één tijdlijn.")
            return
        if not self.output_dir.get():
            tkMessageBox.showerror("Fout", "Geef een doelmap op.")
            return
