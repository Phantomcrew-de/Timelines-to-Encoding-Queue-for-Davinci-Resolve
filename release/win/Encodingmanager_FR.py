...
class RenderGUI:
    def __init__(self, master):
        self.master = master
        master.title("Paramètres de rendu")

        self.current_project = project
        self.render_presets = project.GetRenderPresetList() if project else []

        tk.Label(master, text="Préréglage de rendu:").grid(row=0, column=0, sticky='e')
        ...
        tk.Label(master, text="Motif de recherche de timeline:").grid(row=1, column=0, sticky='e')
        self.search_pattern = tk.Entry(master)
        self.search_pattern.grid(row=1, column=1)
        tk.Button(master, text="Filtrer les timelines", command=self.select_timelines_by_pattern).grid(row=1, column=2)
        ...
        tk.Label(master, text="Dossier de sortie:").grid(row=3, column=0, sticky='e')
        self.output_dir = tk.Entry(master, width=40)
        self.output_dir.grid(row=3, column=1)
        tk.Button(master, text="Parcourir", command=self.browse_dir).grid(row=3, column=2)

        self.use_in_out = tk.IntVar()
        tk.Checkbutton(master, text="Utiliser les marqueurs d'entrée/sortie de la timeline", variable=self.use_in_out).grid(row=4, column=0, columnspan=2)
        ...
        tk.Button(master, text="Démarrer le rendu", command=self.on_start).grid(row=6, column=1, pady=10)
    ...
    def load_timelines(self):
        ...
        for i in range(1, timeline_count + 1):
            ...
            if tl:
                ...
                cb.pack(side="left")
                status = tk.Label(row, text="En attente", width=10)
    ...
    def on_start(self):
        ...
        if not selected_timelines:
            tkMessageBox.showwarning("Avertissement", "Veuillez sélectionner au moins une timeline.")
            return
        if not self.output_dir.get():
            tkMessageBox.showerror("Erreur", "Veuillez spécifier un dossier de sortie.")
            return
