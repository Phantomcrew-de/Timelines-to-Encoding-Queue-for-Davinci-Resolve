...
class RenderGUI:
    def __init__(self, master):
        self.master = master
        master.title("Configuración de renderizado")

        self.current_project = project
        self.render_presets = project.GetRenderPresetList() if project else []

        tk.Label(master, text="Preajuste de renderizado:").grid(row=0, column=0, sticky='e')
        ...
        tk.Label(master, text="Patrón de búsqueda de línea de tiempo:").grid(row=1, column=0, sticky='e')
        self.search_pattern = tk.Entry(master)
        self.search_pattern.grid(row=1, column=1)
        tk.Button(master, text="Filtrar líneas de tiempo", command=self.select_timelines_by_pattern).grid(row=1, column=2)

        self.timeline_vars = {}
        self.status_labels = []
        self.timeline_frame = tk.LabelFrame(master, text="Líneas de tiempo")
        ...
        tk.Label(master, text="Directorio de salida:").grid(row=3, column=0, sticky='e')
        self.output_dir = tk.Entry(master, width=40)
        self.output_dir.grid(row=3, column=1)
        tk.Button(master, text="Explorar", command=self.browse_dir).grid(row=3, column=2)

        self.use_in_out = tk.IntVar()
        tk.Checkbutton(master, text="Usar marcadores de entrada/salida de línea de tiempo", variable=self.use_in_out).grid(row=4, column=0, columnspan=2)
        ...
        tk.Button(master, text="Iniciar renderizado", command=self.on_start).grid(row=6, column=1, pady=10)
    ...
    def load_timelines(self):
        ...
        for i in range(1, timeline_count + 1):
            ...
            if tl:
                ...
                cb.pack(side="left")
                status = tk.Label(row, text="Esperando", width=10)
    ...
    def on_start(self):
        selected_timelines = [i for i, var in list(self.timeline_vars.items()) if var.get() == 1]
        if not selected_timelines:
            tkMessageBox.showwarning("Advertencia", "Por favor selecciona al menos una línea de tiempo.")
            return
        if not self.output_dir.get():
            tkMessageBox.showerror("Error", "Por favor especifica un directorio de salida.")
            return
