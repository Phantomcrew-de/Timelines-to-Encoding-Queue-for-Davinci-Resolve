...
class RenderGUI:
    def __init__(self, master):
        self.master = master
        master.title("Nastavení renderování")

        self.current_project = project
        self.render_presets = project.GetRenderPresetList() if project else []

        tk.Label(master, text="Přednastavení renderování:").grid(row=0, column=0, sticky='e')
        ...
        tk.Label(master, text="Hledat vzor časové osy:").grid(row=1, column=0, sticky='e')
        self.search_pattern = tk.Entry(master)
        self.search_pattern.grid(row=1, column=1)
        tk.Button(master, text="Filtrovat časové osy", command=self.select_timelines_by_pattern).grid(row=1, column=2)

        self.timeline_vars = {}
        self.status_labels = []
        self.timeline_frame = tk.LabelFrame(master, text="Časové osy")
        ...
        tk.Label(master, text="Výstupní složka:").grid(row=3, column=0, sticky='e')
        self.output_dir = tk.Entry(master, width=40)
        self.output_dir.grid(row=3, column=1)
        tk.Button(master, text="Procházet", command=self.browse_dir).grid(row=3, column=2)

        self.use_in_out = tk.IntVar()
        tk.Checkbutton(master, text="Použít značky začátku/konce časové osy", variable=self.use_in_out).grid(row=4, column=0, columnspan=2)
        ...
        tk.Button(master, text="Spustit renderování", command=self.on_start).grid(row=6, column=1, pady=10)
    ...
    def load_timelines(self):
        ...
        for i in range(1, timeline_count + 1):
            ...
            if tl:
                ...
                cb.pack(side="left")
                status = tk.Label(row, text="Čekání", width=10)
    ...
    def on_start(self):
        ...
        if not selected_timelines:
            tkMessageBox.showwarning("Varování", "Vyberte alespoň jednu časovou osu.")
            return
        if not self.output_dir.get():
            tkMessageBox.showerror("Chyba", "Zadejte výstupní složku.")
            return
