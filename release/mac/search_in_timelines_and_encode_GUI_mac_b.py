...
def run_rendering(settings, selected_timelines, log_callback, update_status_callback):
    ...
    if settings.get("start_render", False):
        log_callback("\n[Start] Rendering wird gestartet...\n")
        ...
        log_callback("[Erfolg] Rendering abgeschlossen.\n")
        log_callback("[Dauer] Dauer: %d Sekunden\n" % elapsed)
        log_callback("[Zeitpunkt] Start: %s\n" % datetime.fromtimestamp(start_time).strftime("%H:%M:%S"))
        log_callback("[Zeitpunkt] Ende: %s\n" % datetime.fromtimestamp(end_time).strftime("%H:%M:%S"))
...
class RenderGUI:
    ...
    def update_status(self, index, status_text):
        if index in self.timeline_vars:
            pos = list(self.timeline_vars).index(index)
            self.status_labels[pos].config(text=status_text)
        finished = sum(1 for label in self.status_labels if label.cget("text") == "Fertig")
        total = len(self.status_labels)
        self.progress['value'] = 100.0 * finished / total if total else 0

    def on_start(self):
        selected_timelines = [i for i, var in self.timeline_vars.items() if var.get() == 1]
        if not selected_timelines:
            messagebox.showwarning("Warnung", "Bitte mindestens eine Timeline auswählen.")
            return
        if not self.output_dir.get():
            messagebox.showerror("Fehler", "Bitte Speicherort angeben.")
            return
        ...
        threading.Thread(target=run_rendering, args=(settings, selected_timelines, self.log, self.update_status)).start()
