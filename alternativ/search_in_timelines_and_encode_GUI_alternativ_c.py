...
def run_rendering(settings, selected_timelines, log_callback):
    ...
    for i in selected_timelines:
        ...
        render_settings = {
            ...
            "CustomName": "Render_%s" % timeline.GetName()
        }
...
class RenderGUI:
    def __init__(self, master):
        ...
        tk.Button(master, text="Rendern starten", command=self.on_start).grid(row=5, column=1, pady=10)

        self.log_output = tk.Text(master, height=10, width=80)
        self.log_output.grid(row=6, column=0, columnspan=3)
